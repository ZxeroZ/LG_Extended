...
final class ScreenFloatValueRegEx {
    ...
    static {
        String strStringPlus = Intrinsics.stringPlus("[eE][+-]?", "(\\p{Digit}+)");
        value = new Regex("[\\x00-\\x20]*[+-]?(NaN|Infinity|((" + ("((\\p{Digit}+)(\\.)?((\\p{Digit}+)?)(" + strStringPlus + ")?)|(\\.((\\p{Digit}+))(" + strStringPlus + ")?)|((" + ("(0[xX](\\p{XDigit}+)(\\.)?)|(0[xX](\\p{XDigit}+)?(\\.)(\\p{XDigit}+))") + ")[pP][+-]?(\\p{Digit}+))") + ")[fFdD]?))[\\x00-\\x20]*");
    }
}
