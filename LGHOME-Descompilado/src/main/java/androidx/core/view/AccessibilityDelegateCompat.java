...
public class AccessibilityDelegateCompat {
    ...
    static final class AccessibilityDelegateAdapter extends View.AccessibilityDelegate {
        ...
        public void onInitializeAccessibilityNodeInfo(View host, AccessibilityNodeInfo info) {
            ...
            accessibilityNodeInfoCompatWrap.setScreenReaderFocusable(ViewCompat.isScreenReaderFocusable(host));
            accessibilityNodeInfoCompatWrap.setHeading(ViewCompat.isAccessibilityHeading(host));
            accessibilityNodeInfoCompatWrap.setPaneTitle(ViewCompat.getAccessibilityPaneTitle(host));
            accessibilityNodeInfoCompatWrap.setStateDescription(ViewCompat.getStateDescription(host));
            this.mCompat.onInitializeAccessibilityNodeInfo(host, accessibilityNodeInfoCompatWrap);
            accessibilityNodeInfoCompatWrap.addSpansToExtras(info.getText(), host);
    ...
    public boolean performAccessibilityAction(View host, int action, Bundle args) {
        ...
        while (true) {
            ...
            if (accessibilityActionCompat.getId() == action) {
                zPerformAccessibilityAction = accessibilityActionCompat.perform(host, args);
                break;
            }
            i++;
        }
        if (!zPerformAccessibilityAction && Build.VERSION.SDK_INT >= 16) {
            zPerformAccessibilityAction = this.mOriginalDelegate.performAccessibilityAction(host, action, args);
        }
        return (zPerformAccessibilityAction || action != R.id.accessibility_action_clickable_span) ? zPerformAccessibilityAction : performClickableSpanAction(args.getInt(AccessibilityClickableSpanCompat.SPAN_ID, -1), host);
    }
